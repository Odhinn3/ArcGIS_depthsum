...
class PolygonToProfiles(object):
    ...
    def generate_profiles(polygon_layer, spacing, azimuth, output_fc, utm_zone):
        try:
            ...
            # Если в слое есть выделение — берём экстент выделенного полигона,
            # иначе — экстент всего слоя
            with arcpy.da.SearchCursor(polygon_layer, ["SHAPE@"]) as cursor:
                first_shape = next(cursor)[0]
                centroid = first_shape.centroid

            desc = arcpy.Describe(polygon_layer)
            if hasattr(desc, "fidSet") and desc.fidSet:
                # Вычисляем экстент только по выделенным фичам
                ...
                arcpy.AddMessage("Using extent of selected feature(s).")
            else:
                extent = desc.extent
                arcpy.AddMessage("No selection — using full layer extent.")

            # Фишнет = bbox полигона + 50% с каждой стороны
            pad_x = extent.width  * 0.5
            pad_y = extent.height * 0.5

            fishnet_xmin = extent.XMin - pad_x
            fishnet_ymin = extent.YMin - pad_y
            fishnet_width  = extent.width  + 2 * pad_x   # = extent.width  * 2
            fishnet_height = extent.height + 2 * pad_y   # = extent.height * 2

            n_cols = int(fishnet_width / spacing) + 1

            raw_fishnet = os.path.join(arcpy.env.scratchGDB, "raw_fishnet")
            if arcpy.Exists(raw_fishnet):
                arcpy.Delete_management(raw_fishnet)

            origin_coord = f"{fishnet_xmin} {fishnet_ymin}"
            y_axis_coord = f"{fishnet_xmin} {fishnet_ymin + 10}"

            arcpy.CreateFishnet_management(
                raw_fishnet, origin_coord, y_axis_coord, spacing, fishnet_height, "1", str(n_cols),
                "#", "NO_LABELS", None, "POLYLINE"
            )
